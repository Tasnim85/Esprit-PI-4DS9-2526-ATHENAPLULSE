...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { userData: user } = await loginWithEmail(email, password)
      
      if (user) {
        // Store user info in localStorage
        // The AuthContext will automatically generate and store the JWT
        ...
        } else if (user.role === 'pharmacien' || user.role === 'parapharmacien') {
  ...
  return (
    <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <p style={{ fontSize: '0.75rem', fontWeight: 600, marginBottom: '8px', color: '#0A6EBD' }}>
          🔐 Comptes de démonstration
        </p>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '0.7rem' }}>
          <div>
            <strong>Docteur:</strong><br />
            jean.moreau@cabinet.fr<br />
            Mot de passe: password123
          </div>
          <div>
            <strong>Pharmacien:</strong><br />
            sophie.martin@pharmacie.fr<br />
            Mot de passe: password123
          </div>
          <div>
            <strong>Délégué:</strong><br />
            nicolas.vincent@delegue.fr<br />
            Mot de passe: password123
          </div>
        </div>
      </div>
