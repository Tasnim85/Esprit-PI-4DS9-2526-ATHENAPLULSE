...
const RECOMMEND_API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
const COMPLIANCE_API_URL = process.env.NEXT_PUBLIC_COMPLIANCE_API_URL || 'http://localhost:8001'
...
export default function RecommandationsPage() {
  ...
  const [presentationSlides, setPresentationSlides] = useState<Array<[string, string]>>([])
  ...
  // Health check
  useEffect(() => {
    apiHealth(RECOMMEND_API_URL).then(ok => setIsApiAvailable(ok))
  }, [])
  ...
  useEffect(() => {
    ...
    apiProductSuggestions(RECOMMEND_API_URL, q)
      .then(list => {
        if (cancelled) return
        setSuggestions(list)
  ...
  const submitQuery = useCallback(async (searchQuery: string) => {
    ...
    setProducts([])
    try {
      const results = await apiRecommend(RECOMMEND_API_URL, searchQuery)
      setProducts(results)
      addMessage(
  results.length === 0
    ? {
  ...
  const handleViewDetails = useCallback(async (product: Product) => {
    ...
    try {
      const details = await apiProductDetails(RECOMMEND_API_URL, product.name)
  ...
  const handleOpenPresentation = useCallback(async (product: Product) => {
    console.log('[AthenaPulse] Opening presentation for:', product.name)
    ...
    try {
      const response = await fetch(`${COMPLIANCE_API_URL}/compliance/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      if (data.presentation_slides && Object.keys(data.presentation_slides).length > 0) {
        const slidesArray: [string, string][] = Object.entries(data.presentation_slides).map(
          ([key, value]) => [key, String(value)]
        )
        setPresentationSlides(slidesArray)
      } else {
        const fallbackSlides: [string, string][] = [
          ['Présentation du produit', product.name],
          ['Gamme / Laboratoire', product.brand],
          ['Forme galénique', product.form],
          ['Phase', product.phase],
          ['Indications', product.symptoms.join(', ')],
          ['Score de pertinence', `${(product.score * 100).toFixed(1)}%`],
        ]
        setPresentationSlides(fallbackSlides)
      }
    } catch (err) {
      console.error('[AthenaPulse] Presentation error:', err)
      setPresentationError(err instanceof Error ? err.message : 'Impossible de générer la présentation.')
  ...
  const downloadPPTX = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${COMPLIANCE_API_URL}/compliance/pptx`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          produit: presentationProduct.name 
        }),
      })
      ...
      a.href = url
      a.download = `${presentationProduct.name.replace(/\s+/g, '_')}_presentation.pptx`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
    } catch (err) {
      console.error('[AthenaPulse] PPTX download error:', err)
      alert(`Impossible de télécharger la présentation: ${err instanceof Error ? err.message : 'Erreur inconnue'}`)
    }
  }, [presentationProduct])
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: '0.8rem', color: '#b45309' }}>
                Vérifiez que l'API FastAPI est démarrée sur <code style={{ background: '#fef3c7', padding: '1px 5px', borderRadius: '4px' }}>{RECOMMEND_API_URL}</code>.
              </p>
            </div>
          </div>
        )}
    ...
    {SYMPTOM_SUGGESTIONS.map(({ label, query: q, id }) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          <button
                            type="button"
                            onClick={() => handleOpenPresentation(product)}
                            style={{
                              flex: 1, padding: '8px 0',
                              background: 'linear-gradient(135deg, #0A6EBD, #0D1B2A)',
      ...
      {isPresentationModalOpen && presentationProduct && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label={`Présentation : ${presentationProduct.name}`}
          style={{
            position: 'fixed', inset: 0,
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              <div>
                <p style={{ margin: 0, fontSize: '0.62rem', color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>
                  Présentation produit
                </p>
                <h3 style={{ fontFamily: "'Syne', sans-serif", margin: '2px 0 0', fontSize: '1rem', fontWeight: 700, color: 'white' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {presentationSlides.length > 0 && (
                  <button
                    ...
                    style={{
                      padding: '7px 16px', background: '#05B8CC', color: 'white',
                      border: 'none', borderRadius: '6px', fontWeight: 600,
                      fontSize: '0.78rem', cursor: 'pointer', transition: 'opacity 0.15s',
                    }}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: 'white', display: 'flex', transition: 'background 0.15s',
                  }}
            ...
            <div style={{
              ...
              minHeight: '380px', padding: '40px 48px',
            }}>
              {isPresentationLoading ? (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ color: '#94a3b8', fontSize: '0.875rem', margin: 0 }}>Génération de la présentation…</p>
                </div>
              ) : presentationSlides.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                  <div style={{ fontSize: '48px', marginBottom: '12px', opacity: 0.3 }}>📊</div>
                  <p style={{ color: '#64748b', fontSize: '0.875rem', margin: 0, fontWeight: 600 }}>
                    {presentationError ? '❌ Erreur' : 'Aucune diapositive disponible.'}
                  </p>
                  {presentationError && (
                    <p style={{
                      color: '#ef4444', fontSize: '0.78rem', marginTop: '8px',
                      background: '#fef2f2', border: '1px solid #fecaca',
                      borderRadius: '8px', padding: '10px 16px', maxWidth: '400px', margin: '12px auto 0'
                    }}>
                      {presentationError}
                    </p>
                  )}
                ...
                <div style={{ width: '100%', maxWidth: '780px', textAlign: 'center' }}>
                  <div style={{
                    fontSize: '1.25rem', fontWeight: 700, color: '#0A6EBD',
                    marginBottom: '20px', fontFamily: "'Syne', sans-serif",
                  }}>
                  ...
                  <div style={{
                    ...
                    {presentationSlides[currentSlideIndex][1]}
                  </div>
                </div>
              )}
            </div>
            ...
            {presentationSlides.length > 0 && (
              <div style={{
                ...
                <button
                  ...
                  style={{
                    padding: '7px 18px', background: currentSlideIndex === 0 ? '#444' : '#05B8CC',
                    color: 'white', border: 'none', borderRadius: '6px', fontWeight: 600,
                    fontSize: '0.82rem', cursor: currentSlideIndex === 0 ? 'not-allowed' : 'pointer',
                    opacity: currentSlideIndex === 0 ? 0.45 : 1, transition: 'all 0.15s',
                  }}
                >
                  ← Précédent
                </button>
                  ...
                  {presentationSlides.map((_, i) => (
                    <button
                      ...
                      style={{
                        ...
                        background: currentSlideIndex === i ? '#05B8CC' : '#3a3a3a',
                        color: 'white', border: 'none', borderRadius: '5px',
                        fontWeight: 600, cursor: 'pointer', fontSize: '0.75rem',
                        transition: 'background 0.15s',
                      }}
                      onMouseEnter={e => { if (currentSlideIndex !== i) (e.currentTarget as HTMLButtonElement).style.background = '#555' }}
                      onMouseLeave={e => { if (currentSlideIndex !== i) (e.currentTarget as HTMLButtonElement).style.background = '#3a3a3a' }}
                    >
                      {i + 1}
                    </button>
                  ))}
                ...
                <button
                  onClick={() => setCurrentSlideIndex(i => Math.min(presentationSlides.length - 1, i + 1))}
                  disabled={currentSlideIndex === presentationSlides.length - 1}
                  style={{
                    padding: '7px 18px', background: currentSlideIndex === presentationSlides.length - 1 ? '#444' : '#05B8CC',
                    color: 'white', border: 'none', borderRadius: '6px', fontWeight: 600,
                    fontSize: '0.82rem', cursor: currentSlideIndex === presentationSlides.length - 1 ? 'not-allowed' : 'pointer',
                    opacity: currentSlideIndex === presentationSlides.length - 1 ? 0.45 : 1, transition: 'all 0.15s',
                  }}
                >
                  Suivant →
                </button>
