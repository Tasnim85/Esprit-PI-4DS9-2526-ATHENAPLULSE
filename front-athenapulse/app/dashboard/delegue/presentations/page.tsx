...
export default function PresentationsPage() {
  ...
  // Appel au backend FastAPI (DSO3 sur port 8001)
  ...
  const downloadPPTX = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        console.error(`PPTX download failed: ${response.status}`, errorData)
        return
      }
  ...
  const downloadReport = async () => {
    ...
    try {
      const response = await fetch(`${COMPLIANCE_API_URL}/compliance/report`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert(`Erreur lors de la génération: ${error instanceof Error ? error.message : 'Vérifiez que le serveur DSO3 est démarré sur le port 8001'}`)
    } finally {
