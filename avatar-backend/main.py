...
EXCEL_PATH = os.getenv("EXCEL_DATA_PATH", "extracted_products.xlsx")
...
class ExcelDataLoader:
        ...
        try:
            ...
            for col in self.data.columns:
                ...
                if col_lower == 'product':
                    col_product = col
                elif col_lower == 'indication':
                    col_indication = col
                elif col_lower == 'gamme':
                    col_gamme = col
                elif col_lower == 'composition':
                    col_composition = col
                elif col_lower == 'posologie':
                    col_posologie = col
            
            if not col_product:
                logger.error("Could not find 'Product' column in Excel file")
                return
    ...
    def get_product(self, product_name: str) -> Optional[Dict[str, Any]]:
        ...
        return self.products_dict.get(product_key)
...
class ProductDataRetriever:
    ...
    def get_product_data(self, product_name: str) -> Dict[str, Any]:
        ...
        # Not found in Excel - raise error
        logger.warning(f"Product '{product_name}' not found in Excel")
        raise HTTPException(
            status_code=404,
            detail=f"Product '{product_name}' not found in database"
        )
